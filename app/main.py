...
def import_pazienti(lista: List[PazienteImport]):
    with Session(engine) as session:
        for p in lista: session.add(Paziente(nome=p.nome, cognome=p.cognome, area=p.area))
        session.commit()
    return {"msg": "Pazienti importati"}

@app.post("/import-magazzino")
def import_magazzino(lista: List[InventarioImport]):
    with Session(engine) as session:
        for i in lista: session.add(Inventario(materiale=i.materiale, area_stanza=i.area_stanza, quantita=i.quantita, soglia_minima=i.soglia_minima, obiettivo=i.obiettivo))
        session.commit()
    return {"msg": "Magazzino importato"}

@app.post("/import-prestiti")
def import_prestiti(lista: List[PrestitoImport]):
    with Session(engine) as session:
        for i in lista:
            p = session.exec(select(Paziente).where(Paziente.nome==i.nome_paziente, Paziente.cognome==i.cognome_paziente)).first()
            session.add(Prestito(oggetto=i.oggetto, area=i.area, paziente_id=p.id if p else None, durata_giorni=i.durata_giorni, data_scadenza=date.today()+timedelta(days=i.durata_giorni)))
        session.commit()
    return {"msg": "Prestiti importati"}

@app.get("/")
def home(): return {"msg": "Gestionale Focus Rehab - Sistema Ripristinato"}
