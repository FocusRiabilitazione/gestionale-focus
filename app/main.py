...
# Cambiando nome al file, obblighiamo Railway a crearne uno nuovo pulito.
sqlite_file_name = "database_pulito.db"
...
app = FastAPI(title="Gestionale Focus Rehab - RIPARTENZA")
...
# --- FORMATTAZIONE GRAFICA (ESTERNA PER SICUREZZA) ---
def formatta_con_bottoni(model, attribute):
    stato = ""
    q = model.quantita if model.quantita is not None else 0
    soglia = model.soglia_minima if model.soglia_minima is not None else 2
    obiett = model.obiettivo if model.obiettivo is not None else 5
...
class InventarioAdmin(ModelView, model=Inventario):
    ...
    # Qui colleghiamo la funzione che sta fuori
    column_formatters = {
...
@app.get("/")
def home():
    return {"msg": "Gestionale Focus Rehab - Reset Completato"}
